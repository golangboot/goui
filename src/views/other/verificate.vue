		...
		<el-card shadow="never" header="async-validator内置">
			<el-form ref="form" :model="form" :rules="rules" label-width="100px" status-icon>
				...
				<el-form-item label="异步验证" prop="async">
					<el-input v-model="form.async" placeholder="请输入GoBootAdminUI最新版本号,form开启status-icon后 可以查看右侧状态"></el-input>
				</el-form-item>
				<el-form-item>
					...
					async: [
						{
							...
							validator: (rule, value, callback)=>{
								this.$API.demo.ver.get({value: value}).then(res => {
									if(res.data != value){
										return callback(new Error('请输入GoBootAdminUI最新版本号：'+res.data))
									}
									callback()
								})
							},
